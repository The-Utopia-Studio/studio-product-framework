...
**Venture fellows / outsourced builders:** start at [fellow-day-0.md](./fellow-day-0.md) (env tiers, design system, blueprint). This page is the engineer quick start for the monorepo.
...
- Billing keys as needed (Polar and/or Stripe and/or Autumn) — see env tiers in [fellow-day-0.md](./fellow-day-0.md)
...
- GitHub: install **Greptile** + **Rams** + **Aikido** apps on the repo
...
Fill by tier (A boot → B money → C AI → D ops → E job-specific). Comments in `apps/web/.env.example` and [fellow-day-0.md](./fellow-day-0.md).
...
Fill [engineering-blueprint.md](./engineering-blueprint.md) (80/90 stack defaults).  
Run `agents/loops/integrate-design-system.md` (Utopia DS / designer pack / generate).  
Then `agents/loops/bootstrap-ai-product.md`. First vertical = golden case with [ai-in-workflow.md](./ai-in-workflow.md) (not a chat clone by default).  
Prefer `agents/loops/compound-engineering.md` for features, and `agents/loops/ship-ready-pr.md` before merge (Ponytail → Rams → CI → greploop → Aikido).
...
See [architecture.md](./architecture.md), [capabilities.md](./capabilities.md), [loop-engineering.md](./loop-engineering.md), [fellow-day-0.md](./fellow-day-0.md).  
