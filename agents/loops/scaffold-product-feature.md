...
1. **Clarify non-negotiable** — Does this touch money, inference, or delivery? If yes, plan Effect fence usage.
2. **Locate home** — UI in `apps/web`, orchestration in `apps/web/convex`, shared mechanics in `packages/*`.
3. **Implement thin vertical slice** in the app first (auth → action → UI).
