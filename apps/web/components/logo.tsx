...
// Text wordmark driven by PRODUCT_NAME (see lib/product.ts) rather than a
// fixed brand mark, since this component is shared by every venture stamped
// out of the template — a hardcoded logo would show someone else's brand.
export const Logo = ({
  className,
}: {
  className?: string;
  ...
  return (
    <span
      className={cn(
        "text-foreground truncate font-semibold text-lg tracking-tight",
        className,
      )}
    >
      {PRODUCT_NAME}
